classdef VGVSBody < Body
    %Abstract class modeling a Volumetric continuum under the Geometric Variable Strain (GVS) approach and radius configuration variables.
    %The internal interaction forces are modeled using a linear visco-elastic law.
    %All volume integrals are computed in cylindrical coordinates with a Gaussian quadrature rule.
    ...
    properties
        %Vector collecting the parameters of the GVS Body as :math:`\mathrm{Parameters} = \left( L_{0} \,\, R_{\mathrm{base}} \,\, R_{\mathrm{tip}} \,\, \rho \,\, E \,\, \nu \,\, \eta \,\, N_{\mathrm{Gauss}} \right)^{T} \in \mathbb{R}^{8 \times 1}`, where :math:`L_{0}, R_{\mathrm{base}}` and :math:`R_{\mathrm{tip}}` denote the body rest length, the base and tip radius, :math:`E` is the Young modulus, :math:`\nu`
        %the Poisson ratio, :math:`\eta` the material damping coefficient, and :math:`N_{\mathrm{Gauss}}` represents the number of Gaussian points used for the computation of the kinematics and the volume integrals.
        Parameters;
        RestLength      = 0;
    ...
    methods
        ...
        function Update(obj, q, dq, ddq)
            [T_, omega_, v_, domega_, dv_, J_omega, J_v] = obj.Kinematics(q, dq, ddq);% Update the kinematics for the tip of the body at the center of the backbone
            obj.T_                  = T_;
            obj.v_rel_              = v_;
